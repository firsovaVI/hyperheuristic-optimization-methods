...
def objective_function(x):
    """Целевая функция для оптимизации (минимизация MSE между предсказаниями и данными)"""
    # Генерируем тестовые данные
    test_x = np.linspace(-5, 5, 20)
    true_y = 2 * test_x ** 2 - 3 * test_x + 5  # Истинная парабола

    # Вычисляем предсказания с текущими параметрами
    pred_y = x[0] * test_x ** 2 + x[1] * test_x + x[2]

    # Вычисляем MSE между предсказаниями и истинными значениями
